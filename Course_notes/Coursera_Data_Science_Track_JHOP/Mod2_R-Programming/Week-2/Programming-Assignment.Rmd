...
**I would like to eventually translate this over to Julia** - perhaps over the
weekends and in the free time - or even during dedicated study sessions. 


Download the dataset file.  
`download.file(url = "https://d396qusza40orc.cloudfront.net/rprog%2Fdata%2Fspecdata.zip", "data.zip")`     

## Part 1. 

Create a function consist of three input attributes - a) directory, b) pollutant, and c) id. The function should be called and these attributes provided, after which the function should enter in the directory where the .CSV files are, based on the IDs provided, search the respective .csv for the column name matching the pollutant, and from here, the mean of the row values will be calculated and output as such. Do not round the result. 
...
- Create a logical vector from the complete.cases - each respective column-row will contain TRUE if the cases are complete, and FALSE is the values are missing       
...
## Background notes, sources and helpful links
