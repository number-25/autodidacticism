...
### Cacheing and workflow management 
* There are two main commands for this, the **-resume** which will resume a
previously running workflow which didn't execute entirely due to issues in the
script, and the `nextflow clean [run_name|session_id] [options]` which will
clean the outputs of a run and allow one to start fresh.   
* 
